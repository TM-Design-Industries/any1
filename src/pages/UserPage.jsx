...
// ─── Transactions Tab ────────────────────────────────────────
function TransactionsTab({ userId, navigate }) {
      ...
      {isPublic && txData?.transactions?.map(tx => {
        ...
        return (
          <div key={tx.id} onClick={() => navigate(`/user/${tx.targetId}`)} style={{
            ...
            <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 3, background: isBuy ? '#7A9E7E' : '#C0564A' }} />
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: isBuy ? '#7A9E7E18' : '#C0564A18', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              {isBuy ? <ArrowUpRight size={16} color="#7A9E7E" /> : <ArrowDownLeft size={16} color="#C0564A" />}
            </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.bg, paddingBottom: 40, position: 'relative' }}>
      ...
      {invested && (
        <div style={{
          position: 'fixed', top: 52, right: 16, zIndex: 200,
          background: '#1A2A1A', color: '#7A9E7E',
          border: '1px solid #7A9E7E44', borderRadius: 22,
      ...
      <div style={{ padding: '16px 16px 0' }}>
        ...
        {/* ── TRANSACTIONS TAB ── */}
        {tab === 'transactions' && <TransactionsTab userId={user.id} navigate={navigate} />}
              ...
              <button
                onClick={() => {
                  ...
                  if (existing) {
                    ...
                    portfolio.push({ userId: id, shares: 1, buyPrice: user.marketCap, currentPrice: user.marketCap });
                  }
