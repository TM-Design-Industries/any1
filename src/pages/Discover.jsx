...
export default function Discover({ onSettingsOpen }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.bg, display: 'flex', flexDirection: 'column', fontFamily: "'Inter', -apple-system, sans-serif" }}>
        ...
        <div style={{ display: 'flex', gap: 8, overflowX: 'auto', scrollbarWidth: 'none' }}>
          {[{ id: 'all', label: 'All', color: theme.accent }, ...Object.values(USER_TYPES)].map(t => (
            <button key={t.id} onClick={() => setFilter(t.id)} style={{
              ...
              border: `1px solid ${filter === t.id ? t.color : theme.surface2}`,
        ...
        <div style={{ padding: '16px 0 8px', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
              <button onClick={() => topUser && handlePass(topUser)} style={{ width: 60, height: 60, borderRadius: '50%', background: theme.surface2, border: '1.5px solid #C0564A44', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
              <button onClick={() => topUser && handleSwipeUp(topUser)} style={{ width: 64, height: 64, borderRadius: '50%', background: theme.accent, border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 4px 20px #C9A84C55' }}>
            ...
            {/* Interested - right */}
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
              <button onClick={() => topUser && handleInterested(topUser)} style={{ width: 60, height: 60, borderRadius: '50%', background: theme.surface2, border: '1.5px solid #7A9E7E44', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
      ...
      {showBackModal && backUser && (
        <div style={{ position: 'fixed', inset: 0, background: '#000000cc', backdropFilter: 'blur(10px)', zIndex: 300, display: 'flex', alignItems: 'flex-end' }}>
          <div style={{ width: '100%', maxWidth: 430, margin: '0 auto', background: theme.bg, borderRadius: '24px 24px 0 0', padding: '28px 20px 48px', border: '1px solid #C9A84C33', animation: 'slideUp 0.3s ease', boxShadow: '0 -8px 40px #C9A84C22' }}>
            ...
            {backed ? (
              ...
              <>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                  {['1', '5', '10', '50'].map(v => (
                    <button key={v} onClick={() => setBackAmount(v)} style={{
                      flex: 1, background: backAmount === v ? '#C9A84C22' : theme.surface2,
                ...
                <input type="number" placeholder="Custom amount ($)" value={backAmount} onChange={e => setBackAmount(e.target.value)} style={{ width: '100%', background: theme.surface2, border: '1px solid #3E3528', borderRadius: 12, padding: '13px 16px', color: theme.text, fontSize: 15, outline: 'none', marginBottom: 16, boxSizing: 'border-box' }} />

                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={() => setShowBackModal(false)} style={{ flex: 1, background: theme.surface2, color: theme.muted, border: '1px solid #3E3528', borderRadius: 14, padding: 14, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
