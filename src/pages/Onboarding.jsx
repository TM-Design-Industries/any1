...
const TYPES = [
  {
    id: 'investor', label: 'Investor', color: '#6B9470', desc: 'Back people with capital',
    icon: (
  ...
  {
    id: 'founder', label: 'Founder', color: '#8B85C1', desc: 'Build with vision, own your company',
    icon: (
      <svg width={18} height={18} viewBox="0 0 16 16" fill="none">
  ...
  {
    id: 'surfer', label: 'Surfer', color: '#5AABA2', desc: 'Multi-project, stay flexible',
    icon: (
      <svg width={18} height={18} viewBox="0 0 16 16" fill="none">
  ...
  {
    id: 'expert', label: 'Expert', color: '#B8714F', desc: 'Master of one craft',
    icon: (
      <svg width={18} height={18} viewBox="0 0 16 16" fill="none">
...
export default function Onboarding({ onComplete }) {
  ...
  const handleGo = () => {
    if (!canGo) return;
    const userData = {
      type, name: name.trim(), bio: '', reputation: 32,
      joined: new Date().toLocaleDateString('en-US', { month: 'short', year: 'numeric' }),
      collateral: 3000, investors: 5, marketCap: 8500, missions: 1,
      ...
      location: 'Tel Aviv', color: selectedType?.color || '#C2A45A', tags: [],
    };
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.bg, display: 'flex', flexDirection: 'column', fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }}>
      <div style={{ flex: 1, padding: '60px 24px 24px', display: 'flex', flexDirection: 'column', gap: 36 }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 36, fontWeight: 300, color: theme.text, letterSpacing: '-1px' }}>
            any<span style={{ fontWeight: 900, color: selectedType?.color || '#5AABA2' }}>1</span>
          </div>
        </div>

        <div>
          <div style={{ fontSize: 11, color: theme.muted, marginBottom: 8, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Your name</div>
          <input
            value={name}
            ...
            style={{
              ...
              border: `1px solid ${name ? (selectedType?.color || '#5AABA2') : theme.border}`,
              borderRadius: 14, padding: '14px 16px',
              color: theme.text, fontSize: 16, outline: 'none',
        ...
        <div>
          <div style={{ fontSize: 11, color: theme.muted, marginBottom: 12, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Who are you?</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {TYPES.map(t => (
              <button key={t.id} onClick={() => setType(t.id)} style={{
                background: type === t.id ? `${t.color}12` : theme.surface,
                border: `1.5px solid ${type === t.id ? t.color : theme.border}`,
                ...
                <div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: type === t.id ? t.color : theme.text, marginBottom: 2 }}>{t.label}</div>
                  <div style={{ fontSize: 12, color: theme.muted }}>{t.desc}</div>
                </div>
              </button>
            ))}
          </div>
